<footer class="pt-3 px-4 pb-3">
    <div>
        <p class="font-size-sm">
            <i class="fa fa-copyright"></i> 2020 - 2020 CykloShop
        </p>
        <div class="row mb-3">
            <a href="contact" class="font-size-sm text-dark col-4 d-flex justify-content-center">Kontaktné údaje</a>
            <a href="privacy" class="col-4 font-size-sm text-dark d-flex justify-content-center">
                Ochrana súkromia
            </a>
            <a href="objection" class="font-size-sm text-dark col-4 d-flex justify-content-center">
                Reklamačný poriadok
            </a>
        </div>
        <div class="d-flex justify-content-center">
            <i class="icons fa fa-facebook-square"></i>
            <i class="ml-3 icons fa fa-youtube-square"></i>
            <i class="ml-3 icons fa fa-twitter-square"></i>
            <i class="ml-3 icons fa fa-instagram-square"></i>
        </div>
    </div>
</footer>

<!-- JS bundle w/ Popper.js -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ho+j7jyWK8fNQe+A12Hb8AhRq26LrZ/JpcUGGOn+Y7RsweNrtN/tE3MoK7ZeZDyx" crossorigin="anonymous"></script>